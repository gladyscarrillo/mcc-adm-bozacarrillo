...
matriz_distancia = pd.DataFrame.from_csv('matrices/Test-Largest10K/matrizcompleta.csv')
...
kmedoids = pd.DataFrame.from_csv('clusters/10k/kmedoids.csv')
clusters_universo = pd.DataFrame.from_csv('clusters/10k/cluster_results.csv')
...
for x in range(1, 11):
    print "Muestra %d" % (x)
    startTimeQuery = time.time()
    kmedoids_1 = pd.DataFrame.from_csv('clusters/10k/kmedoids_' + str(x)+'.csv')
...
#    print(resultados)
    acc = (sum(total_aciertos)/(10000*1.0))*100
...
df_acc.to_csv("clusters/10k/acc.csv")
    #print("Tiempo analisis nuevo cluster : %f segundos." % (endTimeQuery - startTimeQuery))
