...
for x in range(1, 11):
    ...
    ran_obj = df_objetos.take(np.random.permutation(len(df_objetos))[:1000])
    ...
    df.columns = ['objeto']
    df.to_csv("clusters/10k/kmedoids_"+str(x)+".csv")
    ...
    df.to_csv("clusters/10k/cluster_results_"+str(x)+".csv")
...
df = pd.DataFrame(lista_kmedoids)
df.columns = ['objeto']
df.to_csv("clusters/10k/kmedoids.csv")
...
df = pd.DataFrame(results)
df.columns = ['cluster','objeto']
df.to_csv("clusters/10k/cluster_results.csv")
